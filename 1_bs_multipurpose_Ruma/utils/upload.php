...
//utilizar $_FILES['file'] no $_FILES['avatar'] por dropzone.js
    ...
    function upload_files()
    {
        ...
        ////////////////////////////////////////////////////////////////////////////
        $upfile = $_SERVER['DOCUMENT_ROOT'].'/php/marvelArte_ORM_4/1_bs_multipurpose_Ruma/media/'.$_FILES['file']['name'];
        if (is_uploaded_file($_FILES['file']['tmp_name'])) {
            if (is_file($_FILES['file']['tmp_name'])) {
                ...
                $upfile = $_SERVER['DOCUMENT_ROOT'].'/php/marvelArte_ORM_4/1_bs_multipurpose_Ruma/media/'.$nombreFichero;
            } else {
                $error .= 'Invalid File...';
            }
        }
        ...
        if ($error == '') {
            if ($copiarFichero) {
                ...
                $upfile = '/php/marvelArte_ORM_4/1_bs_multipurpose_Ruma/media/'.$nombreFichero;

                return $return = array('resultado' => true, 'error' => $error, 'datos' => $upfile);
            }
            if ($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
                $upfile = '/php/marvelArte_ORM_4/1_bs_multipurpose_Ruma/media/default-avatar.jpg';
    ...
    $name = $_POST['filename'];
        if (file_exists($_SERVER['DOCUMENT_ROOT'].'/php/marvelArte_ORM_4/1_bs_multipurpose_Ruma/media/'.$name)) {
            unlink($_SERVER['DOCUMENT_ROOT'].'/php/marvelArte_ORM_4/1_bs_multipurpose_Ruma/media/'.$name);
